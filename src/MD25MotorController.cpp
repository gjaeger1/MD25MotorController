...
{
  if(i2c_write_blocking(this->i2c_instance, MD25ADDRESS, data, num, true) < 0)
    return -1;
...
{
  if(i2c_write_blocking(this->i2c_instance, MD25ADDRESS, &register_address, sizeof(uint8_t), true) < 0)
    return -1;

  if(i2c_read_blocking(this->i2c_instance, MD25ADDRESS, data, (uint8_t) num, false) < 0)
    return -1;
